pipeline {
    ...
    environment {
        ...
        IMAGE_NAME = "group1-team6-eureka-server"
        IMAGE_TAG = "v1.0"
    ...
    stages {
        stage('Checkout') {
            steps {
                ...
                git branch: 'main', url: 'https://github.com/jghan-no1/group1-team6-eureka-server.git'
            }
        }
        ...
        stage('Deployment Image to Update') {
            steps {
                script {
                    ...
                    sh "kubectl set image deployment/eureka-server eureka-server=${REGISTRY}/${IMAGE_NAME}:${IMAGE_TAG} --namespace=${NAMESPACE}"
                }
            }
        }
    }
